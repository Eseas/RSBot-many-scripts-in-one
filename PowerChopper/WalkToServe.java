...
public class WalkToServe extends Task<ClientContext> {
   // private int[] rockIds = {72081, 72082, 72083};
    ...
    private TilePath pathToMines = new TilePath(ctx, Config.PATH).reverse();

    public WalkToServe(ClientContext ctx) {
        super(ctx);
    }
    ...
    public boolean activate() {
        return ctx.backpack.select().count() == 0
                && !ctx.objects.id(Config.objectsIds).nearest().poll().inViewport();
    }
